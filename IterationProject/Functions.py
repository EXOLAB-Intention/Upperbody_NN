...
def process_all_emg_and_velocity(all_trials, fs=100, lpf_fc=5, norm_method='zscore'):
    processed_trials = []
    emg_keys = ['emgL1', 'emgL2', 'emgL3', 'emgL4', 'emgR1', 'emgR2', 'emgR3', 'emgR4']
    imu_keys = ['imu1', 'imu2', 'imu3', 'imu4', 'imu5', 'imu6', 'imu7', 'imu8', 'imu9', 'imu10', 'rhip', 'rknee', 'lhip', 'lknee', 'trunk', 'rshoulder', 'relbow', 'lshoulder', 'lelbow']
...
# Pelvis를 Base[1,0,0,0]로 잡고 prev_joint에 대한 상대 쿼터니언 적용 -> Euler angle (ZYX euler angle -> B3 = Rx * Ry * Rz * B ) "No Library version" + Velocity 계산
def CalibrateIMU_vel(trial):
    quat_raw = {}
    quat_raw["RH_joint"] = np.transpose(trial["imu1"])          # Make (4,T) -> (T,4)
    quat_raw["RK_joint"] = np.transpose(trial["imu2"])  
    quat_raw["LH_joint"] = np.transpose(trial["imu3"])  
    ...
    prev_angle = 0
    euler_rel_vel = {}
    for joint, eulerRel in euler_rel.items():
        euler_rel_vel_seq = []
        for idx, angle in enumerate(eulerRel):
            if (idx == 0):
                euler_rel_vel_temp = [0, 0, 0]
            else:
                euler_rel_vel_temp = (angle - prev_angle) / deltaT
            prev_angle = angle

            euler_rel_vel_seq.append(euler_rel_vel_temp)
        euler_rel_vel[joint] = np.array(euler_rel_vel_seq)

    trial["imu1_vel"] = euler_rel_vel["RH_joint"]
    trial["imu2_vel"] = euler_rel_vel["RK_joint"]
    trial["imu3_vel"] = euler_rel_vel["LH_joint"]
    trial["imu4_vel"] = euler_rel_vel["LK_joint"]
    trial["imu5_vel"] = euler_rel_vel["Pelvis"]
    trial["imu6_vel"] = euler_rel_vel["Trunk"]
    trial["imu7_vel"] = euler_rel_vel["RS_joint"]
    trial["imu8_vel"] = euler_rel_vel["RE_joint"]
    trial["imu9_vel"] = euler_rel_vel["LS_joint"]
    trial["imu10_vel"] = euler_rel_vel["LE_joint"]
