...
%% Additional code for "final.h5"
cnt = 1;

% Left swing
ok_risings_LS = ok_risings(ok_risings < b_risings(cnt));
ok_fallings_LS = ok_fallings(ok_fallings < b_risings(cnt));
cnt = cnt + 2;

% Right swing
ok_risings_RS = ok_risings(ok_risings < b_risings(cnt) & ok_risings > b_risings(cnt-1));
ok_fallings_RS = ok_fallings(ok_fallings < b_risings(cnt) & ok_fallings > b_risings(cnt-1));
cnt = cnt + 2;

% Gait Termination(Left foot)
ok_risings_GT = ok_risings(ok_risings < b_risings(cnt) & ok_risings > b_risings(cnt-1));
ok_fallings_GT = ok_fallings(ok_fallings < b_risings(cnt) & ok_fallings > b_risings(cnt-1));
...
cropped_STEADY_data = {};
APA_time = 1000;        % msec
STEADY_time = 1000;     % msec
Ts = 10;                % msec

for count = 1:size(ok_risings_LS,1)
    start_APA_idx = ok_risings_LS(count);
    stop_APA_idx = start_APA_idx + APA_time/Ts - 1;
    if count < size(ok_risings_LS,1)
        start_STEADY_idx = stop_APA_idx + 1;
        stop_STEADY_idx = start_STEADY_idx + APA_time/Ts - 1;
    end
    ...
    cropped_STEADY.emgL3 = data_emgL3(start_STEADY_idx:stop_STEADY_idx,1);
    cropped_STEADY.emgL4 = data_emgL4(start_STEADY_idx:stop_STEADY_idx,1);
    cropped_STEADY.emgR1 = data_emgR1(start_STEADY_idx:stop_STEADY_idx,1);
    cropped_STEADY.emgR2 = data_emgR2(start_STEADY_idx:stop_STEADY_idx,1);
    cropped_STEADY.emgR3 = data_emgR3(start_STEADY_idx:stop_STEADY_idx,1);
    cropped_STEADY.emgR4 = data_emgR4(start_STEADY_idx:stop_STEADY_idx,1);
    
    cropped_STEADY.rising_ok = rising_edge_ok(start_STEADY_idx:stop_STEADY_idx,1);
    cropped_STEADY.falling_ok = falling_edge_ok(start_STEADY_idx:stop_STEADY_idx,1);
    cropped_STEADY.button_b = rising_edge_b(start_STEADY_idx:stop_STEADY_idx,1);
    
    cropped_STEADY_data{end+1} = cropped_STEADY;
end
...
for i = 1:length(cropped_STEADY_data)
    ...
    for j = 1:numel(fields)
        ...
        % h5create는 처음에만 실행 (존재하면 skip 또는 try-catch)
        if ~isfile(cropped_STEADY_filename) || ~h5exists(cropped_STEADY_filename, path)
            h5create(cropped_STEADY_filename, path, size(data), 'Datatype', class(data));
        end

        % 데이터 저장
        h5write(cropped_STEADY_filename, path, data);
    end
end
disp('SS File is created');
...
for count = 1:size(ok_risings_RS,1)
    start_APA_idx = ok_risings_RS(count);
    stop_APA_idx = start_APA_idx + APA_time/Ts - 1;
    if count < size(ok_risings_RS,1)
        start_STEADY_idx = stop_APA_idx + 1;
        stop_STEADY_idx = start_STEADY_idx + APA_time/Ts - 1;
    end
...
for count = 1:size(ok_risings_GT,1)
    start_APA_idx = ok_risings_GT(count);
    stop_APA_idx = start_APA_idx + APA_time/Ts - 1;
    if count < size(ok_risings_GT,1)
        start_STEADY_idx = stop_APA_idx + 1;
        stop_STEADY_idx = start_STEADY_idx + APA_time/Ts - 1;
    end
